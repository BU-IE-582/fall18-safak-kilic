...
train_glmnet <- function(train_features, test_features,not_included_feature_indices=NULL, alpha=1,nlambda=100, tune_lambda=TRUE,nofReplications=2,nFolds=10,trace=T, cvresults = NULL){
    ...
    if(tune_lambda){
        ...
        cvResultsSummary = list(lambda.min =cv_lambda_min,
                            meanRPS_min=overall_results_summary[lambda==cv_lambda_min]$meanRPS)
    }
    else {
      
      cvResultsSummary = list(lambda.min =cvresults$cv_stats$lambda.min, lambda.1se = cvresults$cv_stats$lambda.1se)
    }
...
train_svm_poly<-function(train_features, test_features,not_included_feature_indices=NULL){
 
  
  train_features$Match_Result<-gsub("Home",1,train_features$Match_Result)
  train_features$Match_Result<-gsub("Away",-1,train_features$Match_Result)
  train_features$Match_Result<-gsub("Tie",0,train_features$Match_Result)
  train_features$Match_Result<-as.numeric(train_features$Match_Result)
  
  svm_features=train_features[complete.cases(train_features)]
  train_class=svm_features$Match_Result
  svm_train_data=svm_features[,-not_included_feature_indices,with=F]
  svm_test_data=test_features[,-not_included_feature_indices,with=F]

  svm_fit_tune<-tune(svm,svm_train_data,as.factor(train_class),kernel="polynomial",ranges = list(gamma = 2^(-1:1), cost = 2^(2:4), degree = c(1,3,5)),tunecontrol = tune.control(cross=10,nboot = 2))

  svm_fit<-svm(svm_train_data,as.factor(train_class),kernel="polynomial",gamma=svm_fit_tune$best.parameters$gamma,degree=svm_fit_tune$best.parameters$degree,cost=svm_fit_tune$best.parameters$cost,probability = TRUE)
 
  final_result<-predict(svm_fit, svm_test_data,probability = TRUE)
  final_result<-cbind(test_features[,1:3],attr(final_result, "probabilities"))
  
  cvResultsSummary = list(gamma=svm_fit_tune$best.parameters$gamma,degree=svm_fit_tune$best.parameters$degree,cost=svm_fit_tune$best.parameters$cost)
  
  return(list(predictions=final_result,cv_stats=cvResultsSummary))
  
}

train_svm_gauss<-function(train_features, test_features,not_included_feature_indices=NULL){
  ...
  svm_fit_tune<-tune(svm,svm_train_data,as.factor(train_class),kernel="radial",ranges = list(gamma = 2^(-1:1), cost = 2^(2:4)),tunecontrol = tune.control(cross=10,nboot = 2))
  
  svm_fit<-svm(svm_train_data,as.factor(train_class),kernel="radial",gamma=svm_fit_tune$best.parameters$gamma,cost=svm_fit_tune$best.parameters$cost,probability = TRUE)
  
  final_result<-predict(svm_fit, svm_test_data,probability = TRUE)
  final_result<-cbind(test_features[,1:3],attr(final_result, "probabilities"))
  
  cvResultsSummary = list(gamma=svm_fit_tune$best.parameters$gamma,cost=svm_fit_tune$best.parameters$cost)
