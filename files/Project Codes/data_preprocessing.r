...
matches_data_preprocessing <- function(data){
  ...
  temp[, c("Home", "Away") := list(sub("-", " ", Home), sub("-", " ", Away))]
  temp[, c("Home", "Away") := list(sub("newcastle utd", "newcastle", Home), sub("newcastle utd", "newcastle", Away))]
  temp[, c("Home", "Away") := list(sub("newcastle united", "newcastle", Home), sub("newcastle united", "newcastle", Away))]
  temp[, c("Home", "Away") := list(sub("manchester utd", "manchester united", Home), sub("manchester utd", "manchester united", Away))]
  temp[, c("Home", "Away") := list(sub("stoke city", "stoke", Home), sub("stoke city", "stoke", Away))]
  ...
  temp[,Match_Day := wday(Match_DateTime)]
  temp[,Match_Month := as.numeric(format(strptime(Match_DateTime, "%Y-%m-%d %H:%M:%OS"), '%m'))]
  ...
  temp[,c('Score','Match_DateTime','AWA_FLAG','POSTP_FLAG','CAN_FLAG',"type", "leagueId"):=NULL]
...
details_data_preprocessing <- function(data,matches,which_bets=c('1x2'),remove_bookmaker=NULL,removeOlderThan=30, handicaplvl = NULL){
  ...
  if(is.null(handicaplvl)) {
    details = details[, totalhandicap := NULL]
  }
  else {
    details = details[(totalhandicap == as.character(handicaplvl)) | is.na(totalhandicap)][, totalhandicap := NULL]
  }
  ...
  setkey(details,bookmaker)
  details<-details[c("BetVictor", "BetWay", "bet365", "ComeOn", "Paddy Power","10Bet","Betfair Exchange","Betclic"),]
